    ...
    try {
      ...
      if (pathname === "/manifest.webmanifest") {
        return new Response(JSON.stringify({
          id: "/inbox.html",
          name: "En-Suite Leads",
          short_name: "En-Suite Leads",
          description: "Website enquiries and live chat for En-Suites & Bathrooms",
...
async function createConversation(request, env) {
  ...
  const welcome = `Hi, welcome to En-Suites & Bathrooms. I can take a few details while I alert a member of our bathroom team.\n\nBefore we start, what’s your first name?`; 
...
async function getCustomerMessages(request, env, conversationId) {
  ...
  return json({
    ...
    name_required: !conversation.customer_name
  });
}
...
async function postCustomerMessage(request, env, conversationId) {
  ...
  const now = new Date().toISOString();
  const awaitingName = Number(conversation.step || 0) === 0 && !conversation.customer_name;
  ...
  if (conversation.mode === "bot") {
    response = buildBotResponse(conversation, body, env);
    nextStep = response.nextStep;
    botMessage = response.message;
  }

  const validNameWasEntered = awaitingName && nextStep === 1;
  const shouldBecomeUnread = !awaitingName || validNameWasEntered;
  ...
  if (shouldBecomeUnread) {
    ...
    statements.push(env.DB.prepare(`
      UPDATE conversations SET updated_at = ?, last_customer_at = ? WHERE id = ?
    `).bind(now, now, conversationId));
  }

  if (response) {
    statements.push(env.DB.prepare(response.updateSql).bind(...response.updateBindings));
    statements.push(env.DB.prepare(`
      INSERT INTO messages (conversation_id, sender, body, created_at)
      VALUES (?, 'bot', ?, ?)
  ...
  if (shouldBecomeUnread && (wasUnread === 0 || validNameWasEntered)) {
    const refreshed = await env.DB.prepare(`SELECT * FROM conversations WHERE id = ?`).bind(conversationId).first();
    if (refreshed?.customer_name) await sendLeadAlert(env, refreshed, body, false);
  }
  ...
  return json({
    ...
    step: nextStep,
    name_required: nextStep === 0,
    customer_name: validNameWasEntered ? normaliseCustomerName(body) : (conversation.customer_name || "")
  });
}
...
function buildBotResponse(conversation, body, env) {
  ...
  if (step === 0) {
    ...
    if (!customerName) {
      return {
        nextStep: 0,
        message: `Before I alert a member of our bathroom team, could I get your first name?`,
    ...
    return botStep(
      ...
      "customer_name",
      customerName,
      `Thanks, ${customerName}. I’m alerting ${owner}, our bathroom specialist, now. He may join the chat while I collect a few details. What are you planning? For example: a new en-suite, bathroom renovation, cloakroom or smaller bathroom work.`
    );
  }
  ...
  if (step === 5) {
    return botStep(id, now, 6, "contact", body, `Thank you. Your enquiry is saved and ${owner}, our bathroom specialist, has been alerted. He may join this same live chat, so you will not need to repeat anything.`);
  }
  return {
    nextStep: step,
    message: `Thanks — I’ve added that to the enquiry. A member of our bathroom team has been alerted.`,
    updateSql: `UPDATE conversations SET updated_at = ? WHERE id = ?`,
    updateBindings: [now, id]
  };
}
...
async function adminLogin(request, env) {
  ...
  return new Response(JSON.stringify({ ok: true, token: value, expires_in: SESSION_SECONDS }), {
    headers: {
      ...JSON_HEADERS,
      "set-cookie": `${ADMIN_COOKIE}=${value}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${SESSION_SECONDS}`
    }
  });
}
...
async function ownerReply(request, env, conversationId) {
  ...
  if (conversation.mode !== "human") {
    statements.push(env.DB.prepare(`
      INSERT INTO messages (conversation_id, sender, body, created_at)
      VALUES (?, 'system', ?, ?)
    `).bind(conversationId, `${env.OWNER_NAME || "Nicholas"} from En-Suites & Bathrooms has joined the chat.`, now));
  }
  statements.push(
    env.DB.prepare(`
      INSERT INTO messages (conversation_id, sender, body, created_at)
...
async function setTakeover(request, env, conversationId) {
  ...
  if (mode === "human") {
    ...
    `).bind(conversationId, `${env.OWNER_NAME || "Nicholas"} from En-Suites & Bathrooms has joined the chat.`).first();
    if (!prior) {
      await env.DB.prepare(`
        INSERT INTO messages (conversation_id, sender, body, created_at)
        VALUES (?, 'system', ?, ?)
      `).bind(conversationId, `${env.OWNER_NAME || "Nicholas"} from En-Suites & Bathrooms has joined the chat.`, now).run();
    }
  }
...
async function sendLeadAlert(env, conversation, latestMessage, reminder = false) {
  ...
  const url = `/inbox.html#chat=${encodeURIComponent(conversation.id)}`;
